...
RSpec.describe CompleteBatch do
  ...
  let(:subject) { CompleteBatch.call(match1.batch, user) }

  context "when there are no remaining matches for the batch" do
    let(:current_instance) { CompleteBatch.new(match1.batch, user) }
    before(:each) do
      match1
      match2
      match3
    ...
    describe "#complete_batch!" do
      it "returns true" do
        ...
        match2.destroy!
        match3.destroy!
        expect(current_instance.complete_batch!).to be_truthy
      end
    end
  ...
  context "when there are remaining matches for the batch" do
    ...
    describe "#complete_batch!" do
      it "returns false" do
        expect(current_instance.complete_batch!).to be_falsey
      end
    end
