...
RSpec.describe ApplicationController, type: :controller do
  ...
  context "user" do
    ...
    context "is not an admin" do
      let(:user) { FactoryBot.create(:user, admin: false, last_activity_at: Time.zone.now) }
    ...
    context "is not a worker" do
      let(:user) { FactoryBot.create(:user, worker: false, last_activity_at: Time.zone.now) }
    ...
    context "is an admin" do
      let(:user) { FactoryBot.create(:user, admin: true, last_activity_at: Time.zone.now) }
    ...
    context "is a worker" do
      let(:user) { FactoryBot.create(:user, worker: true, last_activity_at: Time.zone.now) }
  ...
  context "admin user session has expired" do
    let(:user) { FactoryBot.create(:user, admin: true, last_activity_at: Time.zone.now - 2.days) }
  ...
  context "worker user session has expired" do
    let(:user) { FactoryBot.create(:user, worker: true, last_activity_at: Time.zone.now - 2.days) }
